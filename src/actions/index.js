...
export const fetchSmurfs = () => dispatch => {
    ...
    axios.get('http://localhost:3333/smurfs')
        ...
        .catch(err =>{dispatch({type: FETCH_SMURF_ERROR, payload: err.response})})

}

export const addSmurf = smurf => dispatch =>{
    axios.post('http://localhost:3333/smurfs', smurf)
        .then(res => {dispatch({type: SMURF_NEW, payload: smurf})})
        .catch(err => { dispatch({type:SMURF_NEW_ERROR, payload: err.response})})
